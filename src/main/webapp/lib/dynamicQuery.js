 ...
 * 
 * 1. Connects to the portal via AJAX and downloads a JSON document containing
 * information regarding all cancer studies and gene sets stored in the CGDS. 2.
 * Creates event handler for when user selects a cancer study. This triggers
 * updates in the genomic profiles and case lists displayed. 3. Creates event
 * handler for when user selects a gene set. This triggers updates to the gene
 * set text area.
 ******************************************************************************/
...
$(document).ready(function()
{
  ...
  // Load Portal JSON Meta Data while showing loader image in place of query
  // form
  loadMetaData();
  ...
  // $("#optional_args").hide();
  /*
   * $("#step5_toggle").click(function(event) { event.preventDefault();
...
// Load study Meta Data, i.e. everything except the name, which we load earlier
// to
...
{
  $('#load').remove();
  // show ajax loader image; loader is background image of div 'load' as set in
  // css
...
// search in which the user will enter ONLY a gene list; Also when "All Cancer
// Studies"
...
function reviewCurrentSelections()
{
  ...
  // HACK TO DEAL WITH ASYNCHRONOUS STUFF SO WE DONT DO THIS UNTIL AFTER
  // METADATA ADDED
  if (window.metaDataAdded === true)
  {
    ...
    toggleThresholdPanel($("." + PROFILE_MRNA_EXPRESSION + "[type=checkbox]"), PROFILE_MRNA_EXPRESSION,
        "#z_score_threshold");

    // similarly with RPPA
    toggleThresholdPanel($("." + PROFILE_PROTEIN_EXPRESSION + "[type=checkbox]"), PROFILE_PROTEIN_EXPRESSION,
...
function getMapping()
{
  ...
  function getMap()
  {
    ...
    if (caseIds[0] !== "")
    {
      window.cbioportal_client.getSamplesByPatient({
        study_id: [ studyId
        ],
        patient_ids: caseIds
...
function chooseAction(evt)
{
  ...
  if (selected_studies.length > 1)
  {
    if (haveExpInQuery)
    {
      createAnError("Expression filtering in the gene list is not supported when doing cross cancer queries.",
          $('#gene_list'));
      return false;
    }
    ...
    else
    {
      ...
      var newSearch = $('#main_form').serialize() + '&Action=Submit#crosscancer/overview/' + dataPriority + '/'
          + encodeURIComponent($('#gene_list').val()) + '/' + encodeURIComponent(selected_studies.join(","));
      evt.preventDefault();
...
{
  if (caseSetSelectionOverriddenByUser)
    return;
  ...
  if (mutSelect && cnaSelect && !expSelect && !rppaSelect)
  {
    ...
    {
      defaultCaseList = selectedCancerStudy + "_cna_seq"; // TODO: Better to
                                                          // unify to this one
    }
  }
  ...
  // HACKY CODE START -- TO SOLVE THE PROBLEM THAT WE HAVE BOTH _complete and
  // _3way_complete
...
function toggleThresholdPanel(profileClicked, profile, threshold_div)
{
  ...
  // when a radio button is clicked, show threshold input unless user chooses
  // expression outliers
  if (inputType == 'radio')
  {
  ...
  else if (inputType == 'checkbox')
  {
    ...
    // if there are NO subgroups, show threshold input when mRNA checkbox is
    // selected.
    if (profileClicked.prop('checked'))
    {
      $(threshold_div).slideDown();
    }
    // if checkbox is unselected, hide threshold input regardless of whether
    // there are subgroups
    else
    {
      $(threshold_div).slideUp();
...
function updateCancerStudyInformation()
{
  ...
  addGenomicProfiles(cancer_study.genomic_profiles, "COPY_NUMBER_ALTERATION", PROFILE_COPY_NUMBER_ALTERATION,
      "Copy Number");
  ...
  // addGenomicProfiles(cancer_study.genomic_profiles, "PROTEIN_LEVEL",
  // PROFILE_PROTEIN, "Protein Level");
  addGenomicProfiles(cancer_study.genomic_profiles, "PROTEIN_LEVEL", PROFILE_PROTEIN_EXPRESSION,
  ...
  // Add each case set as an option, and include description as title, so that
  // it appears
  // as a tool-tip.
  jQuery.each(cancer_study.case_sets, function(i, case_set)
  {
    $("#select_case_set").append(
        "<option class='case_set_option' value='" + case_set.id + "' title='" + case_set.description + "'>"
  ...
  // $(".case_set_option").tipTip({defaultPosition: "right", delay:"100",
  // edgeOffset: 25});
  ...
  /*
   * jQuery.getJSON("ClinicalFreeForm.json", {studyId:
   * $("#select_single_study").val()}, function(json){ var noDataToFilter =
   * false;
   * 
   * if (json.freeFormData.length == 0) { noDataToFilter = true; } else {
   * noDataToFilter = true;
   * 
   * var categorySet = json.categoryMap;
   *  // check if there is at least one category to filter for (var category in
   * categorySet) { // continue if the category is qualified as a filter
   * parameter if (isEligibleForFiltering(categorySet[category])) {
   * noDataToFilter = false; break; } } }
   * 
   * if (noDataToFilter) { // no clinical_free_form data to filter for the
   * current
   */// cancer study, so disable the button
  $("#build_custom_case_set").hide();
  /*
   * } else { $("#build_custom_case_set").tipTip({defaultPosition: "right",
   * delay:"100", edgeOffset: 10, maxWidth: 100});
   * 
   * $("#build_custom_case_set").hide();//.show(); temporarily disabled build
   * case list } });
   */
}
...
function cancerStudySelected()
{
  ...
  // make sure submit button is enabled unless determined otherwise by lack of
  // data
...
function addMetaDataToPage()
{
  ...
  // Add studies to tree, and climb up adding one to each level's descendant
  // studies
  ...
  if (window.priority_studies.length > 0)
  {
    for (var i = 0; i < window.priority_studies.length; i++)
    {
      ...
      if (priority_study_obj.studies.filter(function(s)
      {
      ...
      }).length > 0)
      {
        ...
        jstree_data.push({
          ...
          'li_attr': {
            name: priority_study_obj.category
          }
        });
        $.each(priority_study_obj.studies, function(ind, id)
        {
          if (json.cancer_studies.hasOwnProperty(id))
          {
            ...
            jstree_data.push({
              'id': id,
              'parent': category_id,
              'text': studyName.concat('<span style="font-weight:normal;font-style:italic;"> ' + numSamplesInStudy
                  + ' ' + samplePlurality + '</span>'),
              'li_attr': {
                name: studyName,
    ...
    if (currNode.desc_studies_count > 0)
    {
      ...
      $.each(currNode.studies, function(ind, elt)
      {
        ...
        jstree_data.push({
          ...
          'text': name.concat('<span style="font-weight:normal;font-style:italic;"> ' + numSamplesInStudy + ' '
              + samplePlurality + '</span>'),
          'li_attr': {
            name: name,
  ...
  var matchPhrase = function(phrase, node)
  {
    ...
    return !!((node.li_attr && node.li_attr.name && node.li_attr.name.toLowerCase().indexOf(phrase) > -1)
        || (node.li_attr && node.li_attr.description && node.li_attr.description.toLowerCase().indexOf(phrase) > -1) || (node.li_attr
        && node.li_attr.search_terms && node.li_attr.search_terms.toLowerCase().indexOf(phrase) > -1));
  };
  var perform_search_single = function(parsed_query, node)
  {
    // in: a jstree node
    // text to search is node.text and node.li_attr.description and
    // node.li_attr.search_terms
    // return true iff the query, considering quotation marks, 'and' and 'or'
    // logic, matches
    var match = false;
  ...
  var jstree_search = function(query, node)
  {
    ...
    var nodes_to_consider = [ node.id
    ].concat(node.parents.slice());
    var ret = false;
  ...
  var $jstree_flatten_btn = (function()
  {
    ...
    ret.qtip({
      content: {
        text: (jstree_is_flat ? "Unflatten tree" : "Flatten tree")
      },
      style: {
  ...
  var saveSelectedStudiesLocalStorage = function()
  {
    ...
    // $.cookie(selectedStudiesCookieName, selected_studies.join(","),
    // {expires:10});
    return true;
  };
  ...
  var onJSTreeChange = function()
  {
    ...
    $('#jstree_selected_study_count').html(
        (selected_ct === 0 ? "No" : (selected_ct === $('#jstree').jstree(true).num_leaves ? "All" : selected_ct))
            + " stud" + (selected_ct === 1 ? "y" : "ies") + " selected.");
  ...
  {
    $("#select_gene_set").append("<option value='" + key + "'>" + gene_set.name + "</option>");
  }); // end for each gene set loop

  // Set the placeholder for the autocomplete select box
  $("#example_gene_set").children("span:first").children("input:first").attr("placeholder",
      $("#select_gene_set").children("option:first").text());
  ...
  // decodeURIComponent(window.selected_cancer_study_list || (
  // getSelectedStudiesLocalStorage() || '')).split(",");
  var selected_study_list = (window.tab_index !== "tab_download" ? decodeURIComponent(
      window.cancer_study_list_selected || '').split(",") : []);
  ...
  $("#jstree").on('ready.jstree', function()
  {
    ...
    $('#select_single_study').one('doneChanging', function()
    {
      ...
      // To do so, we iterate through all input elements with the name =
      // 'genetic_profile_ids*'
...
function addGenomicProfiles(genomic_profiles, targetAlterationType, targetClass, targetTitle)
{
  ...
  else if (numProfiles > 1 && downloadTab == false)
  {
    ...
    // assign a class to associate the checkbox with any subgroups (radio
    // buttons)
    profileHtml += "<div class='checkbox'><label>" + "<input type='checkbox' class='" + targetClass + "'>"
        + targetTitle + " data." + " Select one of the profiles below:" + "</label></div>";
  ...
  jQuery.each(genomic_profiles, function(key, genomic_profile)
  {
    ...
    if (genomic_profile.alteration_type == targetAlterationType)
    {
      if (downloadTab || genomic_profile.show_in_analysis_tab == true)
      {
        ...
        profileHtml += outputGenomicProfileOption(downloadTab, optionType, targetClass, genomic_profile.id,
            genomic_profile.name, genomic_profile.description);
      }
    }
  }); // end for each genomic profile loop
  ...
  if (targetClass == PROFILE_MRNA_EXPRESSION && downloadTab == false)
  {
    ...
    profileHtml += "<div id='z_score_threshold' class='score_threshold'>" + "<label>Enter a z-score threshold &#177: "
        + "<input type='text' name='" + inputName + "' size='6' value='" + window.zscore_threshold + "'>"
        + "</label></div>";
  }
  ...
  if (targetClass == PROFILE_PROTEIN_EXPRESSION && downloadTab == false)
  {
    ...
    profileHtml += "<div id='rppa_score_threshold' class='score_threshold'>"
        + "<label>Enter a z-score threshold &#177: " + "<input type='text' name='" + inputName + "' size='6' value='"
        + window.rppa_score_threshold + "'>" + "</label></div>";
  }
...
function outputGenomicProfileOption(downloadTab, optionType, targetClass, id, name, description)
{
  ...
  // If we are in the query tab, the input fields must be specified by
  // alteration type.
  // This enforces all the inputs of the same alteration type to work as a
  // single group of radio
  // buttons.
  var paramName;
  if (downloadTab)
  {
  ...
  var html = "<div class='" + optionType + "'>" + "<label>" + "<input type='" + optionType + "'" + " id='" + id + "'"
      + " name='" + paramName + "'" + " class='" + targetClass + "'" + " value='" + id + "' />" + name
      + "  <img class='profile_help' alt='help' src='images/help.png' title='" + description + "'></label></div>";
  return html;
}
